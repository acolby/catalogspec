...
export function renderImplementedItem(instance: SceneItemInstance, implementedCatalog: PreactImplementedCatalog, runtime: RendererRuntimeContext) {
  ...
  for (const [slotName, children] of Object.entries(instance.slots ?? {})) {
    slots[slotName] = children.map((child) => renderImplementedItem(child, implementedCatalog, runtime));
  }
  ...
  const Component = implementedItem;
  return (
    <Component
      key={instance.id}
      {...baseInput}
...
export type RenderImplementedItem = typeof renderImplementedItem;
